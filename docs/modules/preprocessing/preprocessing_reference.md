# Preprocessing — detailed reference

Back to the [module README](preprocessing).
...
- `n_top_genes` (and other HVG params) can be given as a list to sweep multiple values; each combination
  is added as its own `highly_variable-...` column in the assembled output. Only the first value in the
  list becomes the canonical `highly_variable` column and feeds into PCA/neighbors/UMAP.
